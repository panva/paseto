...
const mode = { onlyNative: parameters.has('native'), onlyReference: parameters.has('noble') }
const Native = {
  V1_LOCAL: V1Local,
  V1_PUBLIC: V1Public,
  V2_LOCAL: V2Local,
...
globalThis.pasetoTestResults = { complete: false, total: 0, passed: 0, failed: 0, tests: [] }
...
try {
  const results = await runRuntimeTests({
    ...
    mode,
    onTestComplete(test) {
      const log = test.status === 'passed' ? console.log : console.error
      log(`${test.status === 'passed' ? 'PASS' : 'FAIL'} ${test.name}`)
      if (test.error) console.error(test.error)
    },
  })
  globalThis.pasetoTestResults = { complete: true, ...results }
} catch (error) {
  console.error(error)
  globalThis.pasetoTestResults = {
    complete: true,
    total: 1,
    passed: 0,
    failed: 1,
    tests: [{ name: 'browser harness', status: 'failed', error: error?.stack ?? String(error) }],
  }
}
